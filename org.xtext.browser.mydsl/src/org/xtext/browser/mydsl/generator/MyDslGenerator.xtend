...
class MyDslGenerator extends AbstractGenerator {
	...
	def compile(INSTRUCTION i,int index) '''
		...
		«ENDIF»
		«IF i.class.simpleName == "EndForImpl"»
			«compile(i as EndFor,index)»
		«ENDIF»
	'''
	...
	def compile(Contains c,int index) '''
		...
		«IF (c.type == "Link")»
			//«c.type»
			«IF c.parametre.class.simpleName == "VarImpl"»
			...
			«ENDIF»
			«IF c.parametre.class.simpleName == "ValueImpl"»
				ArrayList<WebElement> linkContains«index» = (ArrayList<WebElement>)  driver.findElements(By.xpath("//a[contains(@title,'" + "«c.parametre.value»" + "')]"));
			«ENDIF»
	...
	def compile(Click c,int index) '''
		...
		«IF (c.type == "Link")»
			//«c.type»
			«IF c.parametre.class.simpleName == "VarImpl"»
				ArrayList<WebElement> linkClick«index» = (ArrayList<WebElement>)  driver.findElements(By.xpath("//a[contains(text(),\""+ «c.parametre.value» +"\")]"));
			«ENDIF»
			«IF c.parametre.class.simpleName == "ValueImpl"»
		...
		«IF (c.type == "Image")»
			...
			if((imageClick«index».size() >= 1)){
				...
				imageClick«index» = (ArrayList<WebElement>)  driver.findElements(By.xpath("//a[contains(@title,'" + "«c.parametre.value»" + "')]"));
				if((imageClick«index».size() >= 1)){
					jse.executeScript("window.scrollTo(" + imageClick«index».get(0).getLocation().x + ", " + (imageClick«index».get(0).getLocation().y - imageClick«index».get(0).getRect().height * 3) + ")");
					imageClick«index».get(0).click();
				}
	...
	def compile(For f,int index) '''
		ArrayList<String> links = new ArrayList<String>();
		«FOR value : f.tableau»
		links.add("«value»");
		«ENDFOR»
		for(String «f.name» : links){
	'''
	
	def compile(EndFor e,int index) '''
